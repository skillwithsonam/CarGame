package com.skillacademy.games;
...
class CarGame extends JFrame implements KeyListener, ActionListener {
	...
	private int tree1ypos = 400, tree2ypos = -200, tree3ypos = -500, tree4ypos = 100, tree5ypos = -300, tree6ypos = 500; // y
																															// position
																															// of
																															// the
																															// obstacles
	...
	private int cypos1 = random.nextInt(5), cypos2 = random.nextInt(5), cypos3 = random.nextInt(5); // y position of the
																									// car
	...
	private boolean rightrotate = false, gameover = false, paint = false; // boolean variables to control the game logic
																			// and the graphics
	...
	public void paint(Graphics g) {
		...
		// draw the road lines
		if (roadmove == 0) {
			for (int i = 0; i <= 700; i += 100) { // draw the road lines
				g.setColor(Color.white);
				g.fillRect(350, i, 10, 70); //

			}
			roadmove = 1; // set the roadmove to 1
		} else if (roadmove == 1) { // draw the road lines again for the next frame
		...
		try {
			tree1 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/tree.png"))); // load the tree image
		} catch (IOException e) {
			e.printStackTrace();
		}

		try {
			tree2 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/tree.png"))); // load the tree image
		} catch (IOException e) {
			e.printStackTrace();
		}

		try {
			tree3 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/tree.png"))); // load the tree image
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		tree6ypos += 50;
		if (tree1ypos > 700) { // if the tree goes out of the screen then reset the tree
			num1 = random.nextInt(500); // generate a random number
		...
		if (tree4ypos > 700) { // if the tree goes out of the screen then reset the tree
			num1 = random.nextInt(500);
			tree4ypos = -num1;
		}
		if (tree5ypos > 700) {
		...
		if (tree6ypos > 700) { // if the tree goes out of the screen then reset the tree
			num1 = random.nextInt(500);
			tree6ypos = -num1;
		}
		...
		// load image for car
		try {
			car = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/gamecar3.png"))); // load the car image
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			car1 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/gamecar1.png")));
		} catch (IOException e) {
			e.printStackTrace();
		}
		// load the opponent image for car
		try {
			car2 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/gamecar2.png")));
		} catch (IOException e) {
			e.printStackTrace();
		}
		// load the opponent image for car
		try {
			car3 = new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("images/gamecar4.png")));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		y2pos += 50;
		y3pos += 50;
		if (y1pos > 700) { // if the opponent car goes out of the screen then reset the opponent car
		...
		if (y2pos > 700) { // if the opponent car goes out of the screen then reset the opponent car
			cxpos2++;
			if (cxpos2 > 4) {
				cxpos2 = 0;
			}
		...
		if (speed > 140) { // if the speed is greater than 140 then reset the speed
			speed = 240 - delay;

		}
		if (score % 50 == 0) { // if the score is divisible by 50 then increase the delay
			delay -= 10;
			if (delay < 60) {
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_LEFT && !gameover) { // if the left key is pressed then move the car to the
																// left
			xpos -= 100;
			if (xpos < 100) {
		...
		if (e.getKeyCode() == KeyEvent.VK_RIGHT && !gameover) { // if the right key is pressed then move the car to the
																// right
			xpos += 100;
			if (xpos > 500) {
		...
		if (e.getKeyCode() == KeyEvent.VK_ENTER && gameover) { // if the game is over and the enter key is pressed then
																// restart the game
			gameover = false;
			paint = false;
			cxpos1 = 0;
	...
	public void keyTyped(KeyEvent e) {
		if (e.getKeyChar() == 'a' && !gameover) { // if the key pressed is 'a' then move the car left
			xpos -= 100; // decrement the xpos by 100

		}
		if (e.getKeyChar() == 's' && !gameover) { // if the key pressed is 's' then move the car right
